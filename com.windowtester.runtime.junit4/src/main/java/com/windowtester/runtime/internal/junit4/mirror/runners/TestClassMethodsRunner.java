...
public class TestClassMethodsRunner extends Runner implements Filterable, Sortable {
	...
	public void run(RunNotifier notifier) {
		if (fTestMethods.isEmpty())
			notifier.fireTestFailure(new Failure(getDescription(), new Exception("No runnable methods")));
	...
	protected void invokeTestMethod(Method method, RunNotifier notifier) {
		...
		try {
			...
			notifier.fireTestFailure(new Failure(methodDescription(method), e.getCause()));
			return;			
		} catch (Exception e) {
			notifier.fireTestFailure(new Failure(methodDescription(method), e));
			return;
		}
