...
public abstract class UITestCaseCommon extends TestCase {
   ...
   */
  private final IApplicationLauncher applicationLauncher;

  /**
   * Manages onetime setup and teardown.
   */
  private static final TestClassManager testClassWatcher =
      new TestClassManager() {
   ...
   *
   * @param launchClass - The application class to be launched by calling the static main method with the specified
   *                    arguments (see ) in a separate thread, or
   ...
   *                    arguments (see ) in a separate thread, or
   *                    <code>null</code> if no application is to be launched.
   * @param launchArgs  - The arguments to be passed to the static main method of the application class to be
   *                    launched, or <code>null</code> if no application is to be launched.
   */
  ...
  public void runUITest(final UITestRunnable runnable) throws Throwable {
    ...
    new SequenceRunner(execMonitor)
        .exec(
            new IRunnable() {
              public void run() throws Throwable {
                runStarted();
  ...
  /**
   * Perform application launching, called before launching. Note: subclasses can participate in launching by
   * attaching listeners here.  For example:
   * <pre>
   * la
   * </pre>
   *
  ...
  /**
   * Create an execution context instance for the current test. It is the subclass responsibility to create the
   * appropriate execution context for its test type.
   */
  ...
  /**
   * Performs one time setup of the test fixture.  Called once per test class, before setup.
   */
  ...
  /**
   * Performs one time teardown of the test fixture.  Called once per test class, after teardown.
   */
