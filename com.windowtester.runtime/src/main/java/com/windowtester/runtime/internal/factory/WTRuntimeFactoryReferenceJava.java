...
/**
 * A facade for an instance of {@link WTRuntimeFactory} when running outside the Eclipse infrastructure such as when
 * testing a Swing application or an SWT application without the OSGi bundle framework.
 */
class WTRuntimeFactoryReferenceJava extends WTRuntimeFactoryReference {
  ...
  static WTRuntimeFactoryReference[] createFactoryReferences() {
    ...
    InputStream stream =
        WTRuntimeFactoryReferenceJava.class.getResourceAsStream(FACTORIES_FILE_NAME);
    try {
  ...
  /**
   * A handler for reading the runtime factory declarations from javaRuntimeFactories.xml
   */
  private static final class FactoryDeclarationHandler extends DefaultHandler {
    ...
    private String getOS(String osName, String osVersion) {
      try {
        String ws = System.getProperty("os.name");
        if (ws.equals("win32")) {
          return "win32";
        }
