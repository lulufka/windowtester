...
/**
 * A condition monitor checks for registered conditions and activates their associated handlers. The order in which
 * conditions are checked is entirely unspecified. Once a condition is satisfied, the associated handler is activated
 * before checking any remaining conditions. Conditions are not checked during the handling of conditions to prevent
 * infinite recursion.
 * <p>
 * Conditions are processed BEFORE all atomic event generating UIContext actions. For example, conditions are checked
 * before the click method in the following code:
 *
 * <pre>
 ...
 *
 * <p>
 * Conditions are added by obtaining this interface from the IUIContext. The {@link
 * com.windowtester.runtime.condition.ICondition#test()} method and the {@link com.windowtester.runtime.condition.IHandler#handle(IUIContext)}
 * method are both called on the WindowTester test thread, so accessing widgets must be through the IUIContext
 * interface, using locator accessor methods such as a implementor of {@link com.windowtester.runtime.condition.IsEnabled#isEnabled(IUIContext)},
 * using {@link com.windowtester.runtime.swt.condition.SWTUIConditionAdapter}, or using {@link
 * org.eclipse.swt.widgets.Display#syncExec(java.lang.Runnable)}. For example:
 *
 * <pre>
 ...
 * </pre>
 * <p>
 * If {@link IConditionMonitor} detects that an {@link ICondition} implements {@link IUICondition}, then {@link
