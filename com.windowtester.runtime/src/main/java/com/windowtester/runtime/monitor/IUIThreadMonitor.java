 ...
 * {@link IUIThreadMonitorListener} (see
 * {@link #setListener(IUIThreadMonitorListener)}) is notified.
 */
public interface IUIThreadMonitor {

  /**
   * Set the listener to be notified if the user interface thread becomes idle or unresponsive for a
  ...
  /**
   * Called to indicate that the UI thread may be idle or unresponsive from this moment for some
   * specified number of milliseconds due some long running operation either on a background thread
   * or in the UI thread itself.
   *
  ...
  /**
   * Set the minimum expected delay between user interface events. If the user interface thread has
   * not processed an event in the specified number of milliseconds, then it is considered either
   * idle or unresponsive too long and an event is triggered.
   *
