...
/**
 * A locator specifying an x,y position. If a locator is specified in the constructor and, during playback, this locator
 * resolves to a widget, the x, y coordinates are used as an offset from the top left corner of the resolved widget.
 * <p>
 * If no locator is specified or the locator is null then the x,y coordinates are relative to the screen itself.
 */
public class XYLocator
     ...
     *
     * @param locator  A locator to which the x,y coordinates are relative or <code>null</code> if the x,y coordinates
     *                 are relative to the screen
     * @param x        the x coordinate
     * @param y        the y coordinate
     * @param relative If, during playback, the locator resolves to an instance of {@link IAreaLocator}, then these
     *                 flags are used to determine how the x,y location is relative to the specified area. This can have
     *                 values of {@link WT#LEFT}, {@link WT#CENTER} or {@link WT#RIGHT} or'd with {@link WT#TOP}, {@link
     *                 WT#CENTER} or {@link WT#BOTTOM} indicating how the x, y coordinates are relative to the specified
     *                 area.
     *                 <p>
     *                 NOTE: relative locators are being hidden for the 2.0 release (to be reviewed for later)
     */
    private /* hidden */ XYLocator(
            ILocator locator,
            int x,
            int y,
            int relative) {
