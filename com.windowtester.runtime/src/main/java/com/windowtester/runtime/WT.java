...
public class WT {
  ...
  /**
   * Set the number of milliseconds to wait for a condition to be true before timing out.
   * <p>
   * This value will be used in all cases where the wait timeout is <em>unspecified</em>, such as:
   * <pre>
   ...
   * <p>
   * This value will be used in all cases where the wait interval is <em>unspecified</em>, such as:
   * <pre>
   * getUI().wait(new ICondition(){...}); //default timeout and interval used
   * </pre>
   ...
   * <p>
   * For eg. If current locale is German / Germany, and the test needs to use the German Keyboard to enter text, then
   * in the test have the statement
   * <p>
   * WT.setLocaleToCurrent();
   * <p>
   * From then on, all text will be entered using the German keyboard. At the end of the tests, in order to set it
   * back to the US Keyboard use
   * <p>
   * WT.resetWTLocale();
   */
