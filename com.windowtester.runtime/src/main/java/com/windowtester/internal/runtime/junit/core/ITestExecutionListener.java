...
public interface ITestExecutionListener {
  ...
  /**
   * The test is finishing. (Note that in situations where test execution is wrapped in its own test
   * thread, this should be called BEFORE leaving that test thread.)
   */
  void testFinishing();

  /**
   * The test is finished. (Note that in situations where test execution is wrapped in its own test
   * thread, this should be called AFTER leaving that test thread.)
   */
  void testFinished();
}
