...
/**
 * A condition monitor checks for registered conditions and activates their associated handlers. See {@link
 * com.windowtester.runtime.condition.IConditionMonitor} for more information.
 */
public class ConditionMonitor implements IConditionMonitor {
  /**
  ...
  /**
   * Flag indicating whether conditions are currently being processed and to prevent recursive checking of conditions.
   * Synchronize against this {@link #_mappings} before accessing this field.
   */
  private boolean _isProcessing = false;

  /**
   * A list of {@link ICondition} / {@link IHandler} pairs maintained and processed by the receiver. Synchronize
   * against this field before accessing either this field or {@link #_isProcessing}.
   */
  private final List _mappings = new ArrayList();

  /**
   * This field is used to cache a copy of the {@link #_mappings} field when processing conditions and associated
   * handlers, and set to <code>null</code> when the cache is invalidated by accessor methods such as {@link
   * #add(ICondition, IHandler)}. Synchronize against {@link #_mappings} before accessing this field.
   */
  ...
  /**
   * Construct a new instance that wrappers the specified parent condition monitor. NOTE: Callers should not access
   * this method directly, but rather use either {@link #getInstance()} to obtain the global condition monitor or
   * obtain the condition monitor local to a particular {@link IUIContext} via {@link IUIContext#getAdapter(Class)} by
   * passing {@link IConditionMonitor} as the argument.
   *
  ...
  /**
   * Answer the global condition monitor. Condition monitors local to a particular {@link IUIContext} can be accessed
   * via {@link IUIContext#getAdapter(Class)} by passing {@link IConditionMonitor} as the argument.
   *
  ...
  /**
   * Utility method for testing conditions. If the specified condition is an instance of {@link IUICondition} then
   * {@link IUICondition#testUI(IUIContext)} is called rather than {@link ICondition#test()}.
   *
   ...
   * processed but no conditions were satisfied, {@link #PROCESS_ONE_OR_MORE} if conditions were processed and at
  ...
  public int process(IUIContext ui) {
    ...
    synchronized (_mappings) {
      ...
      if (_cachedMappings == null) {
        _cachedMappings =
            (ConditionMapping[]) _mappings.toArray(new ConditionMapping[_mappings.size()]);
      }
      ...
      for (int i = 0; i < copyOfMappings.length; i++) {
        if (test(ui, copyOfMappings[i].condition)) {
          TraceHandler.trace(
              ...
              "calling handle: " + copyOfMappings[i].handler);
          try {
  ...
  /**
   * Internal class associating a {@link com.windowtester.runtime.condition.ICondition} with a {@link
  ...
  /**
   * Find the mapping associated with this condition (or <code>null</code> if no associated mapping can be found.
   */
