...
public abstract class WidgetMapper {
   ...
   */
  public WidgetMapper() {
    this(new SmartKeyGenerator());
    /** A default key generator, in case none is specified */
  }
  ...
  /**
   * Retrieve the name tag for the widget associated with this widgetlocator (or <code>null</code> if none was
   * given).
   */
  ...
  /**
   * Returns a set view of the mappings contained in this mapper. Each element in the returned set is a Map.Entry. The
   * set is backed by the mapper, so changes to the mapper are reflected in the set, and vice-versa. If the mapper is
   * modified while an iteration over the set is in progress, the results of the iteration are undefined.
   *
