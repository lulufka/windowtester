...
/**
 * Provides the primary frame for the Costello script editor.  Maintains the LAF used when first created, restoring it
 * temporarily when displaying any new components.
 * <p>
 * FIXME needs major refactoring: Export actions (via ActionMap) Use generic menu setup provided by a special array of
 * actions
 *
...
public class ScriptEditorFrame extends JFrame implements EditorConstants, abbot.Version {
  ...
  private static final int STEP_EDITOR_MIN_HEIGHT = 75;
  ...
  private void setInitialBounds() {
    ...
    Dimension size =
        new Dimension(
            prefs.getIntegerProperty("width", getWidth()),
    ...
    Log.debug("post=" + getBounds() + " split1=" + split1 + " split2=" + split2);
  }
  ...
  public void hide() {
    if (isShowing()) {
      ...
      prefs.setProperty(
          "split.script.stepeditor", String.valueOf(scriptSplit.getDividerLocation()));
      prefs.setProperty(
          "split.script.browser", String.valueOf(scriptBrowserSplit.getDividerLocation()));
      prefs.save();
    }
    super.hide();
  }
  ...
  private void initComponents(ActionListener al) {
    ...
    {
      JLabel suiteLabel =
          new JLabel(getString("AbbotSuite")) {
      ...
      Action action = actionMap.get(ACTION_SELECT_TESTSUITE);
      if (action != null) {
      ...
      action = actionMap.get(ACTION_RUN);
      if (action != null) {
  ...
  private JMenuItem createMenuItem(javax.swing.Action action) {
    JMenuItem item =
        action instanceof EditorToggleAction
            ? new CustomCheckBoxMenuItem((EditorToggleAction) action)
            : new EditorMenuItem(action);
    return item;
  }
  ...
  public void showAboutBox() {
    ...
    aboutBox.setLocation(
        getLocation().x + getWidth() / 2 - aboutBox.getWidth() / 2,
        getLocation().y + getHeight() / 2 - aboutBox.getHeight() * 2 / 3);
    aboutBox.show();
  }
  ...
  public void populateInsertMenu(ArrayList actions) {
    ...
    insertMenu.add(
        assertMenu = new TwoStateEditorMenu("menus.insert-assert", "menus.insert-assert.neg"));
    insertMenu.add(waitMenu = new TwoStateEditorMenu("menus.insert-wait", "menus.insert-wait.neg"));
    ...
    JDialog dialog =
        new JDialog(this, Strings.get("Status.title"), false) {
          public Dimension getMaximumSize() {
            ...
            max.height =
                Math.min(
                    Math.min(max.height, screen.height * 3 / 4),
                    ScriptEditorFrame.this.getMaximumSize().height);
            max.width =
                Math.min(
                    Math.min(max.width, screen.width * 3 / 4),
                    ScriptEditorFrame.this.getMaximumSize().width);
