...
/**
 * Provides extant bug characterization of the current platform.  For any bug encountered, there should be a static
 * method indicating whether the bug is expected on the current platform, for example {@link #showAWTPopupMenuBlocks()}.
...
public class Bugs {
  ...
  public static boolean hasMultiClickFrameBug() {
    ...
    return Platform.isWindows() || Platform.isOSX() || Platform.JAVA_VERSION < Platform.JAVA_1_3;
  }
  ...
  public static boolean hasKeyStrokeGenerationBug() {
    return Platform.isOSX() && Platform.JAVA_VERSION < Platform.JAVA_1_4;
  }
  ...
  public static boolean hasFrameDeiconifyBug() {
    return Platform.isOSX() && Platform.JAVA_VERSION > 0x1310 && Platform.JAVA_VERSION < 0x1424;
  }

  /**
   * OS X (as of 1.3.1, v10.1.5), will sometimes send a click to the wrong component after a mouse move.  This
   * continues to be an issue in 1.4.1
   * <p>
   * Linux x86 (1.3.1) has a similar problem, although it manifests it at different times (need a bug test case for
   * this one).
   * <p>
  ...
  public static boolean hasRobotMotionBug() {
    return Platform.isOSX()
        || (!Platform.isWindows() && Platform.JAVA_VERSION < Platform.JAVA_1_4)
  ...
  public static boolean showAWTPopupMenuBlocks() {
    return Platform.isWindows() && Platform.JAVA_VERSION <= Platform.JAVA_1_5;
  }
  ...
  public static boolean reportsIncorrectLockingKeyState() {
    return Platform.isLinux() && Platform.JAVA_VERSION <= 0x1424;
  }
  ...
  /**
   * Returns whether a {@link java.awt.FileDialog} requires an explicit dismiss (ok/cancel).  Ordinarily {@link
   * Window#dispose()} will work.
   */
  ...
  /**
   * Returns whether a {@link java.awt.FileDialog} misreports its screen location.
   */
  ...
  public static boolean hasTextComponentSelectionDelay() {
    ...
    return (Platform.isLinux() || Platform.isWindows()) && Platform.JAVA_VERSION <= 0x1424;
  }

  /**
   * Check for certain robot-related bugs that will affect Abbot operation.  Returns a String for each bug detected on
   * the current system.
   */
  public static String[] bugCheck(final Window window) {
    if (bugList == null) {
      ...
      if (Platform.isWindows()
          && !Platform.isWindowsXP()
          && Platform.JAVA_VERSION < Platform.JAVA_1_4) {
        Log.debug("Checking for w32 bugs");
        final int mask = InputEvent.BUTTON2_DOWN_MASK;
        MouseAdapter ma =
            new MouseAdapter() {
      ...
      } else if (Platform.isOSX() && Platform.JAVA_VERSION < 0x1430) {
