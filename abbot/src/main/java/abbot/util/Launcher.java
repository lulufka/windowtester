...
/**
 * Mail and browser launcher which augments {@link Runtime#exec} functions.  Provides for built-in email and web browser
 * support.
 */
public class Launcher {
  ...
  private static final String[] HTTP = {
    "galeon", "konqueror", "opera", "firefox", "mozilla", "netscape", "mosaic",
  };

  /**
   * Perform trickery to get the right contents into the email handler from a mailto: line.
   */
  private static String encodeForMail(String base) {
    ...
    String SPACE = "--SPACE--";
    for (int idx = buf.toString().indexOf(" "); idx != -1; idx = buf.toString().indexOf(" ")) {
      buf.replace(idx, idx + 1, SPACE);
    }
    buf = new StringBuffer(URLEncoder.encode(buf.toString()));
    for (int idx = buf.toString().indexOf(SPACE); idx != -1; idx = buf.toString().indexOf(SPACE)) {
      if (Platform.isOSX()) {
   ...
   */
  public static void mail(String user, String subject, String body, String cc, String bcc)
      throws IOException {
