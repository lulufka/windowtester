...
/**
 * Provides a method for terminating threads over which you otherwise have no control.  Usually works.<p> NOTE: Still
 * needs some work; if main script editor exits from the event dispatch thread, an exception is thrown and the exit
 * aborted.  Perhaps ignore event dispatch threads?
 */
public abstract class ThreadTerminatingSecurityManager extends NoExitSecurityManager {
  ...
  private final Map terminatedGroups = new WeakHashMap();
  ...
  private boolean isTerminated(Thread t) {
    ...
    while (iter.hasNext()) {
      ...
      if (thisGroup != null && group.parentOf(thisGroup)) {
        return true;
      }
    }
    return false;
  ...
  /**
   * Ensure ThreadTermination exceptions are thrown for any thread in the given group when any such thread causes the
   * security manager to be invoked.
   */
