...
public class ReferenceAttributeModel extends AbstractTableModel {
  ...
  private static final String[] COLUMN_NAMES = {
    Strings.get("attribute.name"), Strings.get("attribute.value")
  };
  ...
  public int getRowCount() {
    return reference == null ? 0 : reference.getAttributes().values().size();
  }
  ...
  public Object getValueAt(int row, int col) {
    Map.Entry entry = (Map.Entry) reference.getAttributes().entrySet().toArray()[row];
