...
public class JToolBarTester extends JComponentTester {
  ...
  /**
   * Drop the {@link JToolBar} to the requested constraint position, which must be one of the constants {@link
   * BorderLayout#NORTH NORTH}, {@link BorderLayout#EAST EAST}, {@link BorderLayout#SOUTH SOUTH}, or {@link
  ...
  private class JToolBarLocation extends ComponentLocation {
    public Point getPoint(Component c) {
      ...
      int x, y;
      if (Math.max(Math.max(Math.max(insets.left, insets.top), insets.right), insets.bottom)
          == insets.left) {
        x = insets.left / 2;
        y = c.getHeight() / 2;
  ...
  private class DockLocation extends ComponentLocation {
    ...
    public Point getPoint(Component c) {
      ...
      int offset =
          bar.getOrientation() == SwingConstants.HORIZONTAL ? bar.getHeight() : bar.getWidth();
      if (BorderLayout.NORTH.equals(constraint)) {
        x = dock.getWidth() / 2;
        y = insets.top;
