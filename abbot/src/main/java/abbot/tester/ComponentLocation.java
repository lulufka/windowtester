...
/**
 * Provides encapsulation of a visible Component-relative location. "Visible" in this context means currently accessible
 * by the pointer (possibly via scrolling).  A hidden node in a collapsed tree path would <strong>not</strong> be
 * considered visible.
 * <p>
 * This class the specifics of location so that {@link ComponentTester} primitives ({@link
 * ComponentTester#actionClick(Component, ComponentLocation)}, {@link ComponentTester#actionShowPopupMenu(Component,
 * ComponentLocation)}, etc) may be directed to specific elements of substructure on a <code>Component</code> (list
 * rows, tree paths, table cells, substructure values, etc.).
 * <p>
 * Classes derived from <code>ComponentLocation</code> should provide constructors for each type of location indication,
 * e.g. value, index, and {@link Point}.  The {@link #toString()} method should provide an encoded {@link String}
 * suitable for use by the {@link #parse(String)} method, which must convert the {@link String} encoding back into a
 * proper
 * <code>ComponentLocation</code>.
 * <p>
 * By convention, {@link Point} locations are specified with (x,y) notation. Indexed locations should use square
 * brackets, e.g. [i] or [r,c] and value locations should use a quoted {@link String}, e.g. '"cuckoo for cocoa puffs"'.
 * The specific syntax allowed will vary by specific <code>ComponentLocation</code> type.  The base
 * <code>ComponentLocation</code> implementation
 * supports only the explicit (x,y) notation.
 * <p>
 * Recorders should use the {@link String} value by default for consistency.  The special value {@link #CENTER} is
 * provided to indicate the center of a {@link Component}.
 * <p>
 * The method {@link #badFormat(String)} should provide usage-like information indicating the acceptable forms of input
 * for this class.
 *
 * @see JListLocation
 * @see JTreeLocation
