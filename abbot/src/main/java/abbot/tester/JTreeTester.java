...
public class JTreeTester extends JComponentTester {
     ...
     */
    public static boolean isLocationInExpandControl(
            JTree tree,
            int x,
            int y) {
     ...
     * visible.
     */
    public static TreePath pathToStringPath(
            JTree tree,
            TreePath path) {
        if (path == null) {
            return null;
    ...
    public void actionToggleRow(
        ...
        if (tree.getToggleClickCount() != 0) {
            actionClick(tree, loc, InputEvent.BUTTON1_DOWN_MASK,
     ...
     */
    public boolean assertPathExists(
            Component tree,
            TreePath path) {
        try {
