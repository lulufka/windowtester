 ...
 * which uses a fixture step (as opposed to an explicit launch) will only instantiate the fixture if it does not already
 * exist.<p> A Fixture will only be run once for any consecutive group of
 * <code>Script</code>s that refer to it.  The {@link StepRunner} class is
 * normally used to control execution and will manage fixture setup/teardown as needed.
 */
