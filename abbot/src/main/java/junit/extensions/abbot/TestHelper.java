...
/**
 * Provides automatic test suite generation given command-line arguments. Also allows for a single test to be run if it
 * is specified.  Supports "-v" (verbose) to display each test's name prior to running the test (aids in identifying
 * which test failed when the VM crashes). The locale may be set explicitly by the following system properties:
 * <ul>
...
public class TestHelper {
    ...
    protected static Test collectTests(
            String[] args,
            Class testClass)
        ...
        if (args.length == 1 && args[0].startsWith("test")) {
            try {
                ...
                test = (Test) testClass.getConstructor(String.class).newInstance(new Object[]{args[0]});
            }
        } else {
            try {
