 ...
 * <p>
 * By default, generate a suite of all scripts found in a given directory for which the accept method returns true. Note
 * that there is no guarantee of the order of the scripts.
 * <p>
 ...
 * preserves existing environment windows (e.g. the JUnit Swing UI TestRunner) and disposes of all windows generated by
 * the code under test.  Derived fixtures may provide arbitrary code in their setUp/tearDown methods (such as
 * install/uninstall a custom security manager, set system properties, etc), the same as you would do in  any other
 * derivation of junit.framework.TestCase.
 * <p>
 * <h3>Example 1</h3>
 * Following is a ScriptTestSuite which will aggregate all tests in the directory "src/example", whose filenames begin
 * with "MyCode-" and end with ".xml":<br>
 * <pre><code>
...
public class ScriptTestSuite extends TestSuite {
    ...
    /**
     * Constructs a suite of tests from all the scripts found in the directory specified by the system property
     * "abbot.testsuite.path". The most common use for this constructor would be from an Ant 'junit' task, where the
     * system property is defined for a given run. The suite will recurse directories if "abbot.testsuite.path.recurse"
     * is set to true.
     */
    ...
    /**
     * Constructs a suite of tests from all the scripts found in the current directory.  Does not recurse to
     * subdirectories.  The Class argument must be a subclass of junit.extensions.abbot.ScriptFixture.
     */
    ...
    /**
     * Constructs a suite of tests from all the scripts found in the given directory.  Does not recurse to
     * subdirectories. The Class argument must be a subclass of junit.extensions.abbot.ScriptFixture.
     */
    public ScriptTestSuite(
    ...
    /**
     * Constructs an ScriptTestSuite from all the scripts in the given directory, recursing if recurse is true. The
     * Class argument must be a class derived from junit.extensions.abbot.ScriptFixture.
     */
    public ScriptTestSuite(
            final Class<?> fixtureClass,
            final String dirname,
        ...
        if (!primaryDirectory.exists() || !primaryDirectory.isDirectory()) {
            final String msg = "Directory '" + dirname + "' did not exist" + " when scanning for test scripts";
    ...
    /**
     * Constructs a suite of tests for each script given in the argument list, using the given class derived from
     * ScriptFixture to wrap each script.
     */
    public ScriptTestSuite(
            final Class<?> fixtureClass,
        ...
        String msg = "Loading " + fixtureClass + ", with "
    ...
    /**
     * Add all test scripts in the given directory, optionally recursing to subdirectories.  Returns a list of absolute
     * paths.
     */
