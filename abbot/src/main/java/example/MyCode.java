...
public class MyCode {
  ...
  public static void main(String[] args) {
    ...
    for (int i = 0; i < 20; i++) cb.addItem("Combo " + i);
    pane.add(cb);
    ...
    String[] myListData = {"zero", "one", "two", "three", "four", "five", "six", "seven", "eight"};
    ...
    JTextArea ta =
        new JTextArea(
            "Four score and seven hundred years ago, our forebears extended claws reaching from the"
                + " innermost mind to the outer limits",
            10,
            20);
    ...
    KeyStroke ks = KeyStroke.getKeyStroke(KeyEvent.VK_I, KeyEvent.ALT_MASK);
...
class DropLabel extends JLabel {
  ...
  public DropLabel(String name, boolean accept) {
    ...
    dropTarget = new DropTarget(this, DnDConstants.ACTION_COPY_OR_MOVE, dtl, true);
  }
}
...
class DragLabel extends DropLabel {
  ...
  private final int acceptedActions = DnDConstants.ACTION_COPY_OR_MOVE;
  private Color oldColor = null;

  public DragLabel(String name) {
    this(name, true);
  }
  ...
  public DragLabel(String name, final boolean acceptDrops) {
    ...
    dgl =
        new DragGestureListener() {
          public void dragGestureRecognized(DragGestureEvent e) {
            ...
            try {
              e.startDrag(
                  acceptDrops ? DragSource.DefaultCopyDrop : DragSource.DefaultCopyNoDrop,
                  new DragData(),
                  dsl);
              oldColor = getForeground();
    ...
    dragSource.createDefaultDragGestureRecognizer(this, acceptedActions, dgl);
  }
}
