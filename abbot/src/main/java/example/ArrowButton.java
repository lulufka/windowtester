...
public class ArrowButton extends JPanel {
  ...
  public void paint(final Graphics g) {
    ...
    g.setColor((in && down) ? getBackground().brighter() : getBackground().darker());
    if (direction == LEFT) {
      g.drawLine(w - 2, 2, w - 2, h - 2);
      g.drawLine(w - 2, h - 2, 2, (h - 2) / 2);
      g.setColor((in && down) ? getBackground().darker() : getBackground().brighter());
  ...
  protected void fireActionEvent() {
    if (listeners != null) {
      final ActionEvent ev = new ActionEvent(this, ActionEvent.ACTION_PERFORMED, direction);
